...
import TriStateSwitchField from '~/components/forms/fields/switch/TriStateSwitchField.vue'
...
const factory = (props: Record<string, unknown> = {}) => {
    return mount(TriStateSwitchField, {
        props: {
            ...defaultProps,
            ...props
        }
...
describe('TriStateSwitchField', () => {
    ...
    it('applies correct size and switch styling classes', () => {
        const wrapper = factory({
            ...
            modelValue: TriStateValue.CHECKED
        })
    ...
    it('emits update:modelValue with CHECKED when toggled from INDETERMINATE', async () => {
    ...
    it('does not validate when required is false', async () => {
        ...
        await wrapper.setProps({ modelValue: TriStateValue.UNCHECKED })
    ...
    it('has role="switch" and aria-checked="mixed" when indeterminate', () => {
        const wrapper = factory({ modelValue: TriStateValue.INDETERMINATE })
        const switchEl = wrapper.find('[role="switch"]')

        expect(switchEl.exists()).toBe(true)
        expect(switchEl.attributes('aria-checked')).toBe('mixed')
    })
